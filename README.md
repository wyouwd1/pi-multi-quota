...
| **OpenCode Go**（Zen） | rolling / weekly / monthly 百分比 | 复用 pi 已配置的 API key |
...
装完重启 pi 或 `/reload`。卸载：`pi remove /home/user/workspace/pi-multi-quota`
...
| `/quota all` | 一次查询全部供应商（Ark 每个账号 + Zen + DeepSeek），并排对比 |
...
3. 回到 pi 运行 `/quota set ark-a` → 粘贴 → 回车
4. 扩展会**立刻发一次真实请求**校验：
...
- `provider`：**必须与 pi 里的 provider id 完全一致**，扩展靠它把账号绑定到对应的 provider
...
| `Ark-A ✗ 未配置` | 该槽位 cookie 为空 | `/quota set ark-a` |
...
- 只向白名单 host 发送凭据：`console.volcengine.com` / `opencode.ai` / `api.deepseek.com`；**发送前**校验
- 拒绝跟随重定向，防止凭据被转发到其他 host
...
设计与决策记录见 [`SPEC.md`](./SPEC.md) 与 [`tasks/TEAM-SYNC.md`](./tasks/TEAM-SYNC.md)。
